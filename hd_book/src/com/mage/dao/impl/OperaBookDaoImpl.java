...
public class OperaBookDaoImpl implements OperaBookDao {
	 ...
	 */
	@Override
	public List<OperaBook> findBorrowBooksByKey(int currentPage,String key) throws SQLException {
		QueryRunner runner = new QueryRunner(JDBCUtil.getDataSource());
		return runner.query("select * from operatebook where bookname like '%"+key+"%' or username like '%"+key+"%' limit ? , ?", new BeanListHandler<OperaBook>(OperaBook.class) , (currentPage-1)*PAGE_SIZE, PAGE_SIZE);
	}
...
//		return runner.query(sql, new BeanListHandler<OperaBook>(OperaBook.class),list.toArray());
	...
	@Override
	public PageBean findBorrowByKey(int currentPage, String key)throws SQLException {
		...
		pageBean.setPageSize(pageSize); //设置每页显示多少记录
		OperaBookDao dao = new OperaBookDaoImpl() ;
		List<OperaBook> list =dao.findBorrowBooksByKey(currentPage,key);
		pageBean.setList(list); 
		//总的记录数， 总的页数。
		int count = dao.findCount();
		pageBean.setTotalSize(count); //设置总的记录数
