...
public class ActivityController {
    ...
    public @ResponseBody Object saveCreateActivity(Activity activity, HttpSession session){
        ...
        //封装参数  //前台其他的六个属性已经封装过了
        activity.setId(UUIDUtils.getUUID());
        activity.setCreateTime(DateUtils.formateDateTime(new Date()));
        activity.setCreateBy(user.getId());

        ReturnObject returnObject=new ReturnObject();
        try {   //写数据可能会改变其他数据,从而报异常
    ...
    public @ResponseBody Object queryActivityByConditionForPage(String name,String owner,String startDate,String endDate,
        ...
        map.put("beginNo",(pageNo-1)*pageSize);   //beginNo代表的是当前页(pageNo)中第一条记录在数据库中属于第几条记录
        map.put("pageSize",pageSize);             //pageSIze代表一页显示的记录条数
        //调用service层方法，查询数据
    ...
    @RequestMapping("/workbench/activity/fileDownload.do") //这个方法没有用到
    public void fileDownload(HttpServletResponse response) throws Exception{  //下载文件的话返回值类型设为void
        //1.设置响应类型   这里使用application/octet-stream,是根据文件类型来定义的,网上搜content-Type对照表
        ...
        InputStream is=new FileInputStream("D:\\framework\\crm_ssm\\resources\\Excel\\student.xls");
        ...
        //关闭资源
        is.close();
        out.flush();  //out不关闭,out是response内部new的,后边也许还需要用,这里不管
    }
    ...
    //===================================自写===========================================================
    @RequestMapping("/workbench/activity/exportAllActivitysByIds.do")
    public void exportAllActivitysByIds(String[] ids,HttpServletResponse response) throws Exception{
        //调用service层方法，查询所有的市场活动
        List<Activity> activityList=activityService.queryAllActivitysByIds(ids);
        //创建exel文件，并且把activityList写入到excel文件中
        HSSFWorkbook wb=new HSSFWorkbook();
        HSSFSheet sheet=wb.createSheet("市场活动列表");
        HSSFRow row=sheet.createRow(0);
